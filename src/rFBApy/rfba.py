...
def update_regulatory_state(
    ...
    mext_state = {m: 0 if w[m] == 0 else 1 for m in mn.metabolites(True, False) if m in bn}
    r_state = {r: 0 for r in mn.reactions() if v[r] == 0 and r in bn}
...
def update_kinetics(
    ...
    for n in w_:
        ...
        w_[n] = w_[n] = w_[n] - ((s_mr * v[r]) / growth) * biomass * (1 - exp(growth * tau * k))
        w_[n] = max(0., w_[n])
    return w_
...
def estimate_metabolite_depletion_ts(
    ...
    return floor(
        log(1 - ((w[m] * v["Growth"]) / (s_mr * v[r] * biomass)))
            / (v["Growth"] * tau)
    )
...
def estimate_mn_state_duration_ts(
    ...
    return min(
        estimate_metabolite_depletion_ts(mn, obj, m, tau, biomass, w, v)
        for m in w
    )
...
def estimate_state_duration(
    ...
    w_plus_1: dict[str, float] = update_kinetics(
        mn, tau, biomass_plus_1, v[obj], w, v, k=1,
    )
    x_plus_1: dict[str, int] = update_regulatory_state(
        mn, bn, w_plus_1, v, x,
    )
    if x_plus_1 != x:
        return 1
...
def next_iter(
    ...
    next_w: dict[str, float] = update_kinetics(
        mn, tau, biomass, v[obj], w, v, duration,
    )
    next_x: dict[str, int] = update_regulatory_state(mn, bn, next_w, v, x)

    bounds: dict[str, tuple[float, float]] = update_bounds(
        mn, tau, next_biomass, next_w,
    ) | {
        n: (0., 0.) for n in bn if n in mn.reactions() and next_x[n] == 0
    }
    _, next_v = fba.solve(bounds)

    next_duration: int = estimate_state_duration(
        mn, bn, tau, next_biomass, next_x, next_v, next_w, obj,
    )
...
def simulate_rfba(
    ...
    bn: BooleanNetwork = BooleanNetwork.load(bnet)
    ...
    x: dict[str, int] = bn(
        {n: 0 for n in bn.keys() if n not in mn.metabolites(True, False)} | {
            n: 1 if w.get(n, 0) > 0 else 0
            for n in bn.keys()
            if n in mn.metabolites(True, False)
        } | state 
    )
    ...
    while i < iter + 1:
        ...
        v_1, w_1, x_1, biomass_1, duration_1 = next_iter(
            mn, bn, fba, obj, tau, v_0, w_0, x_0, biomass_0, duration_0,
        )
    ...
    # Post-processing
    # --------------------------------------------------------------------------
    met_cols: dict[str, str] = {m: f"{m} {TAG_METABOLITE}" for m in mn.metabolites(True, False)}
    react_cols: dict[str, str] = {r: f"{r} {TAG_REACTION}" for r in mn.reactions()}
    reg_cols: dict[str, str] = {n: f"{n} {TAG_REGULATION}" for n in bn if n not in mn.metabolites()}
    ...
    for s in range(0, len(simulation_states)):
        ...
        for i in range(duration_0) if not compressed else {0, duration_0 - 1}:
            ...
            w_i: dict[str, float] = update_kinetics(mn, tau, biomass_0, growth_0, w_0, v_0, i + 1)
            state_i: dict[str, float | int] = {
                "Time": time_rounded, "biomass": biomass_i,
            } | {
    ...
    cols: list[str] = ["Time", *sorted(met_cols.values()), *sorted(react_cols.values()), *sorted(reg_cols.values()), "biomass"]
